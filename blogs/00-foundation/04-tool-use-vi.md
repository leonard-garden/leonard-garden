# Tool use / Function calling — LLM học cách gọi công cụ bên ngoài
...
**Q4**: RAG inject context *trước* — phù hợp với knowledge base tĩnh, semantic search. Tool use LLM *chủ động hỏi* — phù hợp với real-time data, side effects. Kết hợp được: tool `search_kb` fetch documents → result làm grounding context.
